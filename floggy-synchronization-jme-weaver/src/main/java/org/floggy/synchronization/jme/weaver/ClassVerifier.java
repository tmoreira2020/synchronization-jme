   ...
   */
	...
	*/
	public boolean isModified() throws NotFoundException {
		CtClass synchronizableClass =
			classPool.get(Weaver.__SYNCHRONIZABLE_CLASSNAME);

		return ctClass.subtypeOf(synchronizableClass);
	}

	/**
	* Check if the class implements the persistable interface
	* (net.sourceforge.floggy.Synchronizable).
	*
	* @return True if the class implements such interface, false otherwise.
	*
	* @throws NotFoundException
	*/
	public boolean isSynchronizable() throws NotFoundException {
		CtClass synchronizableClass =
			classPool.get(Weaver.SYNCHRONIZABLE_CLASSNAME);
